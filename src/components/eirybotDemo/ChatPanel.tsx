...
export default function ChatPanel({ sessionId, onFinish }: ChatPanelProps) {
    ...
    const changeLanguage = async (lang: 'es' | 'en') => {
        ...
        try {
            ...
            await fetchSession(); // Refresh to ensure sync
        } catch (e) {
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex bg-black/5 p-1 rounded-lg gap-1">
                            <button
                                ...
                                className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all ${(!session || session.language === 'es') ? 'bg-white shadow text-slate-800' : 'text-slate-400 hover:bg-black/5'}`}
                            >
                                ES
                            </button>
                            <button
                                onClick={() => changeLanguage('en')}
                                className={`px-2 py-1 rounded-md text-[10px] font-bold transition-all ${(session?.language === 'en') ? 'bg-white shadow text-slate-800' : 'text-slate-400 hover:bg-black/5'}`}
                            >
                                EN
                            </button>
                        </div>
